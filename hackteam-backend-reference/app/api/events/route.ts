...
function shapeEvent(e: { id: string; canonicalName: string; organizer: string }, platformName: string) {
  return { id: e.id, name: e.canonicalName, organizer: e.organizer, platform: platformName };
}
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(
    events.map((e) => shapeEvent(e, e.links[0]?.platform.name ?? "Unlisted"))
  );
}
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json(shapeEvent(event, platform.name));
}
